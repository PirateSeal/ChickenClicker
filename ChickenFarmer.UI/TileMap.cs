...
namespace ChickenFarmer.UI
{
    ...
    {
        private const int UnderLayer = 2; // define how many layers of the tmx are draw under the player

        private static readonly Vector2f[] Direction =
        {
            new Vector2f(0 + 0.0075f, 0 + 0.0075f), // 0.0075f  fix texture bleeding
            new Vector2f(16 + 0.0075f, 0 + 0.0075f), new Vector2f(16 + 0.0075f, 16 + 0.0075f),
        ...
        public void Draw(IRenderTarget target, in RenderStates states)
        {
            ...
            foreach ( VertexArray item in UnderMap ) item.Draw(target, state);
        }
        ...
        private void ConvertLayers()
        {
            ...
            foreach ( TmxLayer layer in _map.Layers )
            {
                ...
                if ( layerIdx < 2 )
                    UnderMap[layerIdx ++] = vertexArrayUnder;
                else
                    OverMap[-2 + layerIdx ++] = vertexArrayOver;
            }
        }

        public void Dispose()
        {
            foreach ( VertexArray item in UnderMap )
            {
                item?.Dispose();
                Disposed = true;
            }
        ...
        public void DrawOver(IRenderTarget target, in RenderStates states)
        {
            ...
            foreach ( VertexArray item in OverMap ) item.Draw(target, state);
        }

        public void ConvertCollide(Vector pos) { GameCtx?.FarmUI.Farm.CollideCollection.AddObject(pos, 16, 16); }
        ...
        private void Add(Vector2i vertexPos, int gid, VertexArray vertexArray, Color color)
        {
            ...
            vertexArray.Append(new Vertex(Direction[0] + ( Vector2f ) vertexPos, color,
                                          new Vector2f(tu * TileSize, tv * TileSize)));

            vertexArray.Append(new Vertex(Direction[1] + ( Vector2f ) vertexPos, color,
                                          new Vector2f((tu + 1) * TileSize, tv * TileSize)));

            vertexArray.Append(new Vertex(Direction[2] + ( Vector2f ) vertexPos, color,
                                          new Vector2f((tu + 1) * TileSize, (tv + 1) * TileSize)));

            vertexArray.Append(new Vertex(Direction[3] + ( Vector2f ) vertexPos, color,
